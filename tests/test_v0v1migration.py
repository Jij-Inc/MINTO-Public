...
def test_migration_with_sampleset(temp_dir, v0_experiment_with_sampleset):
    """Test migration with SampleSet objects."""
    # Migrate to v1
    exp_v1 = migrate_to_v1_from_v0(v0_experiment_with_sampleset)

    # Verify sampleset migration
    run_table = exp_v1.get_run_table()
    assert run_table.loc[0, ("parameter", "temperature")] == 1.0

    # Check if sampleset was migrated to the appropriate storage
    assert len(exp_v1.runs[0].samplesets) > 0

    # Verify specific sampleset properties
    sampleset_name = list(exp_v1.runs[0].samplesets.keys())[0]
    sampleset = exp_v1.runs[0].samplesets[sampleset_name]

    # Verify the sampleset was correctly converted to ommx format
    assert hasattr(sampleset, "raw")

    # Check if problem was also migrated
    assert any("problem" in key for key in exp_v1.runs[0].problems.keys())
...
def test_old_sampleset_migration():
    sampleset = jm.SampleSet(
        record=jm.Record(
            solution={
                "x": [(([0, 1, 1], [1, 0, 2]), [1.0, 2.0, 3.0], (2, 3))],
            },
            num_occurrences=[1],
        ),
        evaluation=jm.Evaluation(
            energy=[-1.0],
            constraint_violations={"constraint": [0.0]},
        ),
