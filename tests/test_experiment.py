...
def test_sampleset_for_openjij(tmp_path):
    ...
    A = parametric_qubo.get_parameters()[0]
    ...
    assert loaded_exp.dataspace.run_datastores[0].samplesets["tsp"] == sampleset
...
def solver_with_instance(instance: ommx_v1.Instance):
    ...
    model.optimize()
    return scip_ad.model_to_solution(model, instance)
...
def test_list_parameter_logging(experiment):
    ...
    assert stored_object["paramter_list_param"] == test_list
...
def test_dict_parameter_logging(experiment):
    ...
    assert stored_object["paramter_dict_param"] == test_dict
...
def test_numpy_array_parameter_logging(experiment):
    ...
    assert np.array_equal(stored_object["paramter_array_param"], test_array)
...
def test_nested_parameter_logging(experiment):
    ...
    assert stored_object["paramter_nested_param"] == nested_data
