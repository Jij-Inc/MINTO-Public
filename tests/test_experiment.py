...
def test_auto_saving_experiment_data(tmp_path):
    """auto_savingがTrueの時、experiment-levelのデータがファイルに保存されることを確認"""
    # auto_saving=Trueで実験を作成
    ...
    exp.log_global_config("knapsack", data)
    exp.log_global_instance("knapsack", instance)
    exp.log_global_parameter("test_param", 42)

    # ファイルが存在することを確認
    exp_dir = tmp_path / exp.experiment_name / "experiment"
    assert exp_dir.exists(), "Experiment directory should exist"

    # 各データファイルが存在することを確認
    problem_file = exp_dir / "problems_knapsack.problem"
    assert problem_file.exists(), f"Problem file should exist: {problem_file}"

    instance_file = exp_dir / "instances_knapsack.instance"
    assert instance_file.exists(), f"Instance file should exist: {instance_file}"

    object_file = exp_dir / "objects_knapsack.json"
    assert object_file.exists(), f"Object file should exist: {object_file}"

    param_file = exp_dir / "parameters_.json"
    assert param_file.exists(), f"Parameters file should exist: {param_file}"

    # ファイルの内容が空でないことを確認
    assert problem_file.stat().st_size > 0, "Problem file should not be empty"
    assert instance_file.stat().st_size > 0, "Instance file should not be empty"
    assert object_file.stat().st_size > 0, "Object file should not be empty"
    assert param_file.stat().st_size > 0, "Parameters file should not be empty"
    ...
    with open(param_file, "r") as f:
        params = json.load(f)
        assert "test_param" in params, "test_param should be in parameters file"
        assert params["test_param"] == 42, "test_param value should be 42"


def test_auto_saving_run_data(tmp_path):
    """auto_savingがTrueの時、run-levelのデータがファイルに保存されることを確認"""
    # auto_saving=Trueで実験を作成
    exp = minto.Experiment(
        name="test_auto_save_run",
        savedir=tmp_path,
        auto_saving=True,
        collect_environment=False,
    )
    ...
    run = exp.run()
    with run:
        run.log_parameter("value", 123)
        run.log_solution("knapsack", solution)

    # ファイルが存在することを確認
    run_dir = tmp_path / exp.experiment_name / "runs" / "0"
    assert run_dir.exists(), f"Run directory should exist: {run_dir}"

    # 各データファイルが存在することを確認
    param_file = run_dir / "parameters_.json"
    assert param_file.exists(), f"Parameters file should exist: {param_file}"

    solution_file = run_dir / "solutions_knapsack.solution"
    assert solution_file.exists(), f"Solution file should exist: {solution_file}"

    # ファイルの内容が空でないことを確認
    assert param_file.stat().st_size > 0, "Parameters file should not be empty"
    assert solution_file.stat().st_size > 0, "Solution file should not be empty"
    ...
    with open(param_file, "r") as f:
        params = json.load(f)
        assert "value" in params, "value should be in parameters file"
        assert params["value"] == 123, "value should be 123"
