...
The `Experiment` class allows users to log problems, instances, solutions, parameters,
and other data to a database. It supports automatic saving of experiment data and
provides methods for loading and saving experiments from/to various sources.
...
class Experiment:
    ...
    def log_data(
        ...
        If the current experiment is not running, the data is saved to
        `.dataspace.experiment_datastore`.
        If the current experiment is running, the data is saved to
        `.dataspace.run_datastores[self._run_id]`.
    ...
    def log_instance(
        ...
        If instance_name is not specified, it will be named sequentially as
        "0", "1", "2", etc.
    ...
    def log_parameter(
        ...
        Args:
            ...
            value (Union[float, int, str, list, dict, numpy.ndarray]):
        ...
        Raises:
            ValueError: If a complex data structure (list, dict, numpy.ndarray) is not
                JSON serializable. This can happen if the structure contains
                objects that
        ...
        Note:
            - For complex data structures (list, dict, numpy.ndarray), the value
              is stored
            ...
            - If the experiment is running (i.e., within a run context), the parameter
              is saved with the current run ID. Otherwise, it is saved as
        ...
        Examples:
            ...
            >>> exp.log_parameter(
            ...     "model_config", {"activation": "relu", "dropout": 0.5}
            ... )
    ...
    def log_solver(
        ...
        *,
        ...
        - Each keyword argument passed to the solver function is logged as a
          parameter if it is of type int, float, or str.
        - If a keyword argument is of type `jm.Problem`, it is logged as a problem.
        - If a keyword argument is of type `ommx_v1.Instance`, it is logged as an
          instance.
        - After the solver function executes, the result is logged as a
          sampleset if it is of type `ommx_v1.SampleSet`.
    ...
    def runs(self) -> list[DataStore]:
        ...
        This property is a syntax sugar for accessing the run datastores in the
        experiment.
